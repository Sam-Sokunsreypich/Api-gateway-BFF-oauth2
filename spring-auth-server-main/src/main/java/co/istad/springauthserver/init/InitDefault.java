...
//        Init user
        ...
        User user = User.builder()
                .uuid(UUID.randomUUID().toString())
                .roles(List.of(role))
                .email("sokunsreypichsam@gmail.com")
                .password(passwordEncoder.encode("password"))
                .username("sokunsreypich")
                .familyName("sam")
                .givenName("sokunsreypich")
                .isEnabled(true)
                .build();
        log.info("INITIALIZED USER : {}", userRepo.save(user));

//        String encodedSecret = passwordEncoder.encode("gateway-secret");
